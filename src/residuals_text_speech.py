...
def residuals_phones(source_train, source_val, y_train, y_test):
    trim = 5  
    np.random.seed(9)
    ...
    test_res = (np.nan_to_num(y_test) - y_pred)
    
    y_pred = np.dot(np.nan_to_num(sourcedata_train), weights)
    train_res = (np.nan_to_num(y_train) - y_pred)
    # final_residuals = np.concatenate([train_res, test_res])

    return [train_res, test_res]

def residuals_textual(source_train, source_val, y_train, y_test):
    trim = 5  
    np.random.seed(9)
